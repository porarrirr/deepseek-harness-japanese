...
describe('directory-picker-browse client half', () => {
  ...
  it('registers the dialog dictionaries and binds this package namespace', async () => {
    ...
    b.ctx.get('locale')!.setLocale('ja')
    expect(injected.t('browser.title')).toBe('ワークスペースのディレクトリを選択')
    expect(injected.t('browser.newFolder')).toBe('新しいフォルダー')
    expect(injected.t('browser.showHidden')).toBe('隠しファイルを表示')
  })
