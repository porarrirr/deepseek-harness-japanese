...
describe('LanguageRow', () => {
  ...
  it('opens the menu, selects a locale, and closes', () => {
    ...
    expect(screen.getByRole('menuitem', { name: '日本語' })).toBeDefined()
    fireEvent.click(screen.getByRole('menuitem', { name: '日本語' }))
    expect(b.setLocale).toHaveBeenCalledWith('ja')
    expect(trigger.getAttribute('aria-expanded')).toBe('false')
    expect(screen.queryByRole('menuitem', { name: '日本語' })).toBeNull()
  })
  ...
  it('follows store changes; an unknown active id falls back to the id itself', () => {
    const b = mount('en')
    act(() => { b.store.actions.sync('ja', OPTIONS, 1) })
    expect(screen.getByRole('button', { name: /日本語/ })).toBeDefined()
    act(() => { b.store.actions.sync('zh', OPTIONS, 2) })
    expect(screen.getByRole('button', { name: /中文/ })).toBeDefined()
    act(() => { b.store.actions.sync('fr', OPTIONS, 3) })
