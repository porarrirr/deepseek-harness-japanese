...
**This skill is guidance, not a translation memory.** It is the workflow map for keeping non-public `foo.md ↔ foo.zh.md` pairs and published `foo.md ↔ foo.zh.md ↔ foo.ja.md` pairs consistent and natural. All recorded languages carry equal authority — a change is authored in any one side, and that side is the source for that update. You are the translator: the rules below say what must hold, not how to phrase any particular sentence — phrasing judgment is yours, terminology is not.
...
1. Switcher: for a non-public pair, `[English](foo.md) | 中文` immediately after the Chinese file's H1 and `English | [中文](foo.zh.md)` after the English file's H1; for a published pair, use `English | [中文](foo.zh.md) | [日本語](foo.ja.md)` in English, `[English](foo.md) | 中文 | [日本語](foo.ja.md)` in Chinese, and `[English](foo.md) | [中文](foo.zh.md) | 日本語` in Japanese. Add the required lines when this is a new pair, except that a generator-owned English source stays byte-identical to generator output and omits its switcher while translated counterparts still link back.
