...
- **`verify-translation-pairing` 加入 `doc-sync`。** 门禁（[scripts/verify-translation-pairing.ts](../../../../scripts/verify-translation-pairing.ts)）按照发布清单推导的语言集合强制执行规则：每个已发现且未排除的源文档都有完整双语配对；如果英文锚点出现在 `website/docs.ts` 的 `docsPages` 中，则必须有完整三语配对。每个现有配对都完整且一致（所有必需所有者文件、严格数量的 hash、规范切换行和一致的结构签名）；被排除的生成文档、指令文档或本身即双语的文档不得配对。[scripts/translation-pairing.manifest.json](../../../../scripts/translation-pairing.manifest.json) 只包含显式排除项，因此任何要求都无法绕过发现流程而接受较弱的检查。只有当 `.zh.md` 或 `.ja.md` 围栏序列与其无后缀兄弟文件拥有顺序相同、正文按字节一致的同一组受跟踪围栏时，面向源码的代码门禁才会将其作为派生内容消费；不完整、顺序变更、重分类或已改动的序列仍会独立受检，因此由其所属的代码门禁或配对门禁报告不匹配。
- **语言集合推导。** 范围内的每篇文档都必须拥有由 `docsPages` 选择的完整语言集合：公开页面是三语，其他文档是双语。政策没有逐文件推进状态、日期分界或 README 专用类别。README 发现会覆盖 vendor 源码、依赖目录与被忽略的构建产物目录之外所有文件名不区分大小写匹配 README 的文件，包括今后新增的顶层目录。发布到文档站的配对使用 `pairedPages()`，由根 locale 投影 `.zh.md`，由 `/en/` 投影 `.md`；仅创建对侧文件并不会发布它。[公共日语配对 Agent Note](2026-08-20-public-japanese-translation-pairing.md)负责记录这一拆分及其后果。
...
- 修改已配对文档的任一侧，同一个 PR 就有义务更新其他必需一侧并重新记录配对。门禁将 doc-sync 规则改为按语言集合执行，不变式由 CI（而非评审者的记忆）承载。
...
- 生成的英文文档仍由源码派生，并由各自的生成器实施新鲜度门禁。有经评审中文与日语对侧的公开生成页面遵循四文件配对工作流，但有一项结构例外：生成的英文源文件不含语言切换行，因为添加该行会使生成器新鲜度检查失败；译文对侧仍链接回其他所有语言。没有经评审对侧的生成页面在必需对侧落地前保持为不完整的公开配对。
