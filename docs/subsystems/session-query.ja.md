...
[English](session-query.md) | [中文](session-query.zh.md) | 日本語

live-preferred logical session corpusに対するquery vocabularyです。[Service Definition package](../../packages/session-query/session-query)はexact read、source precedence、relationship tracing、semantic extraction、provider-independent filterを所有し、[SQLite provider](../../packages/session-query/session-query-sqlite)は具体的なfull-text index lifecycleを所有します。

Source: [`packages/session-query/session-query/src/types.ts`](../../packages/session-query/session-query/src/types.ts)

## Logical record

`SessionRecord`はcross-corpus listが返すrecordです。cloneされたlive-preferred headerとは独立してsource availabilityを公開します。`SessionEventRecord`は軽量なraw-log projectionで、classificationにはmodel-history derivationと同じ`foldSurface()` transitionを使います。
...
`SessionLogSnapshot`はresume preflightが使う、完全にdetachedでreplay-validatedなraw logです。`SessionSurfaceSnapshot`はretained subscriptionではなく、1回のexact-readによるsurface observationです。
...
`SessionTitleObservation`はtitle foldingにも同じatomic-observation ruleを適用します。そのためauthorization consumerはtitleを提供したsource headerをvalidateできます。batch readはuniqueなrequested idごとに順序付きの`SessionTitleObservationResult`を1つ返します。operational failureはそのidに留まり、cancelはoperation全体をrejectします。
...
## Provider-independent filterとdocument

session filterとevent filterのarrayはANDし、1つのlist clause内のvalueはORします。rangeはinclusiveです。eventの`text` clauseは、full-text providerに依存せず、抽出したsemantic textに対するliteral Unicode case-insensitiveでwhitespace-flexibleなregular-expression scanです。
...
`ctx.sessionQuery.filterSessions(filters)`はcomplete logical corpusに`SessionResultFilter`を適用し、`ctx.sessionQuery.filterEvents(sessionId, filters)`は一致するdocumentをascending seq orderで返します。message、reasoning、tool call／result、blocked prompt、todo、failure／status detailはsemantic textに寄与しますが、structural eventとstream chunkは寄与しません。

## Full-text search

統合された`ctx.sessionQuery` seamには2つのfull-text scopeがあります。`searchSessions()`は最も強く一致するeventでcorpusをgroupし、`searchEvents()`は1つのsessionをsearchします。requestはopaque cursorをnormalized query、metadata filter、limitにbindします。event text scanは意図的にprovider metadata filterには含めません。
...
group化したcross-session hitとは異なり、session内searchはpageにhitがない場合でもobserved target headerを公開しなければなりません。
...
`SessionLineageTrace`は既知のparentをimmediateから外側の順に運び、recursiveにnestedしたdirect descendantのforestを持ちます。completeness discriminantにより、既知のrootとmissing parentは相互排他的です。
...
requestは1つのraw seqとoptionalな前後件数を指定します。resultはavailability flagではなく`SessionHeader`を持つため、既知のlive targetはpersistence healthから独立できます。
...
event traceはpositional surface replacementと、sourceとして引用されたeventを区別します。すべてのseq listはdirect linkを含みます。ただし`replacementChain`はtargetからfinal positional replacementまでimmediate replacerをたどります。
...
closed code unionはrequest validation、missing target、malformed surface log、optional backend failure、deploymentでdisabledなsearch、矛盾するsource metadataを区別します。
