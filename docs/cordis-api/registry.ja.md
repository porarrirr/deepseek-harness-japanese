...
[English](registry.md) | [中文](registry.zh.md) | 日本語


plugin loadingとdependency injection。
...
要求したserviceが利用可能になったらcallbackを1回実行します。

`ctx.plugin({ inject, apply: callback })`のshort-handです。required serviceが変わるたびにcallbackをunloadして再実行します。

- `deps` — arrayまたはname → config mapで指定するrequired service。
- `callback` — `(ctx, config)`で呼び出すplugin body。

**Returns** fiber。awaitするとloading完了後にsettleします。
...
current contextにpluginをloadします。

- `plugin` — function、class、または`{ apply }` object plugin。
- `args` — plugin config。`Config` schemaに対してvalidateされます。

**Returns** fiber。awaitするとloading完了後にsettleします（configまたはstartup errorではrejectします）。

[Source](../../vendor/cordis/src/registry.ts#L185)

## Plugin

サポートされるplugin entrypointの形式。
...
pluginと`@Inject` decoratorが受け付けるservice dependency declaration。

array形式はintercept configなしでserviceを要求します。object形式は各service nameをplugin context向けのoptional intercept configにmapします。
