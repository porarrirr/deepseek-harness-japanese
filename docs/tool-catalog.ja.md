...
[English](tool-catalog.md) | [中文](tool-catalog.zh.md) | 日本語


出荷済みpluginが`ctx.tools`に提供する、すべてのmodel向けtoolを掲載します。system-prompt assemblyを通じてmodelが受け取る`name`、`description`、JSON-Schema `parameters`です。[subsystem pages](subsystems/core.md)（型と各ページの生成Cordis API領域）を補完するもので、このページにはagentに提供される*tool*を記載します。

このファイルはGENERATEDで、`pnpm run verify-tool-catalog`（`doc-sync`の一部）によってfreshnessを検証します。手動で編集しないでください。純粋なsource-AST passであるCordis catalogとは異なり、このgeneratorは実際のcontextで各tool pluginをBOOTし、`ctx.tools.schemas()`を読み取ります。tool schemaは静的に決定できないためです（runtimeで展開されるenum、連結されたdescription、configで決まるname、raw JSON SchemaのMCP toolなど）。completeness guardは`packages/*/tool-*`をglobし、generatorのboot manifestにpackageが1つでも欠けていれば失敗します。そのため新しいtoolが黙って未文書化になることはありません。[tool-schema-catalog Agent Note](../.agents/notes/implemented/process/2026-07-02-tool-schema-catalog.md)を参照してください。

対象は`packages/*/tool-*`配下の出荷済みproduct toolです。各toolはDEFAULT configでbootします。ただし、defaultのないREQUIREDなConfig fieldがある場合はgeneratorが値を選び、packageごとのnoteにこのページがどのbranchを示すかを記録します。登録されるtool NAMEはload-time config（例：`tool-subagent`の`toolName`）にできるため、deploymentがpackageを別名または追加名で公開する場合があります。出荷されたaliasがある場合はpackageごとのnoteに記録します。`examples/`のdemo tool（例：`echo`）は、Cordis catalogがpackageだけを対象とすることに合わせて除外します。

## Tool Package Map

この表は、modelから見えるtool nameと、その背後にあるplugin packageおよびservice seamを対応付けます。正確なJSON Schemaは、以下のpackage sectionに続けて記載します。

| Tool package | Modelから見えるname | 必要なservice | 書き込み／影響 | 出荷されたalias | 配備時の注記 |
| --- | --- | --- | --- | --- | --- |
