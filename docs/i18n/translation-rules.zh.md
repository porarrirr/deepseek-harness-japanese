...
- 语体以 [style-samples.md](style-samples.md) 为校准锚点。人工定稿的金标样例按文体各一组，译文必须参照文体最接近的样例，采用其中目标语言一侧的语体；如果样例与本文的行文规则冲突，以样例为准。译成中文时，采用规范的技术制度文；译成日语时，采用简洁自然的技术日语；译成英文时，采用简洁、专业的开发者文档语体。
...
- 翻译为中文时，类别名词使用中文并在首现括注英文（实操手册（cookbook））；翻译为日语时，使用术语表中的日语译法，不添加中文括注；翻译为英文时，使用通行的英文类别名。指目录或文件本身时保留代码体英文。
...
- 链接与锚点：每个相对链接在所有语言文件中必须指向相同的目标（按约定是 `.md` 路径而非任何译文兄弟文件），这样即使某对文档先于相邻文件落地，链接也不会悬空。只有语言切换行使用语言特有链接。在 GitHub 以外位置渲染的 README 可以按 [README.md](README.md) 的规定，使用指向确切对侧文件的规范公开仓库 URL。普通链接的**文字**翻译；目标不翻。

本仓库的 Markdown 约定对 `.zh.md` 与 `.ja.md` 文件原样生效：一个段落一个物理行（`verify-md-wrap`）、相对链接必须可解析（`verify-md-links`）、文件末尾恰好一个换行。

## 术语

- [terminology.md](terminology.md) 是各方向的术语真源。翻译前请先加载它；表内术语必须遵守对应行与「不要译作」禁项。译成中文时，采用「中文」列，并按「首次出现」列括注；译成日语时，采用日语章节；译成英文时，采用「English」列，不加中文或日语括注。
...
- 日语正文必须使用日语常用标点（`、。：「」`），不得把中文正文在拉丁词或数字周围使用的全角空格规则带入日语。
- 产品名、API 名、命令、flag、文件路径和行内代码必须保留规范拉丁形式，不得音译标识符。
- 同一篇文档内应统一使用日语括号和句界，不要让中文与日语标点承担同一类正文作用。

## 质量标准

- 一对文档的完成标准：一位双语工程师只读其中任一文件，能获得与另一文件读者完全相同的信息（相同的事实、相同的告诫、相同的语气），并且没有任何多余的内容。
- 请运行 `pnpm run verify-translation-pairing` 与 `doc-sync` 的其余门禁。这些门禁会检查一致性记录、切换行、标题深度、代码块、表格行列数、列表类型、有序列表起始编号、列表项数量、链接及仓库 Markdown 规则；列表与表格的顺序、非常规列表编号、行内代码、强调标记、语义、术语和各语言语体则由人工评审负责。
