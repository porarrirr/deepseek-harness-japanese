...
# Capability seamとコアサービス

[English](capability-seams.md) | [中文](capability-seams.zh.md) | 日本語


サービスは、コアのspineサービス、交換可能なcapability seam、またはbundle／構成ポイントになり得ます。このグラフは、サービス宣言を所有するパッケージ、既知の実装パッケージ、サービスを直接利用するパッケージを示します。
...
メンテナンスモード：ハイブリッド。サービスはCordis宣言から検出し、インターフェース／実装／利用側の役割は完全性ガード付きで`scripts/gen-doc-graphs.ts`が分類します。
