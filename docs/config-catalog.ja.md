...
[English](config-catalog.md) | [中文](config-catalog.zh.md) | 日本語


`cordis.yml` entryで設定できるすべての`config:` blockを掲載します。load可能な各harness packageについて、`apply` functionまたはservice constructorが受け取るconfig declaration（JSDocを含む）をそのまま示し、参照される型もすべて併記します（package内の型は貼り付け、その他はリンク）。貼り付けるのはpluginが宣言する完全なconfig typeです。runtime schemaが意図的に除外するfieldはruntime-only seam（自身のJSDocにその旨が記載されています）であり、`cordis.yml`からは設定できません。これは**deployment**軸のreferenceです。plugin authorが扱うwireは各[subsystem page](subsystems/core.md)の生成Cordis API領域に、model向けtool schemaは[tool catalog](tool-catalog.md)に、これらのdeclarationが参照する型は[subsystems/](subsystems/core.md)に記載されています。

このファイルはsource（`scripts/gen-config-catalog.ts`）からGENERATEDされ、`pnpm run verify-config-catalog`（`doc-sync`の一部）でfreshnessを検証します。手動で編集しないでください。declaration blockは`ts config-catalog` fenceを使います（importを参照する単独のdeclarationはstandalone compileできないため、doc-typecheckはskipします）。generatorはruntime schemastery schemaと貼り付けたdeclarationもcross-checkします。nested keyを含むschema-validated keyはすべて宣言済みconfig type上に見つからなければならず、貼り付けによってloaderが受け入れるfieldを隠せないようにしています。

`Requires:`行にはpluginが`inject`するservice keyを列挙します。その`cordis.yml` treeでは、それらのserviceのproviderもloadしなければなりません。対象はharness tier（`packages/`）です。config treeがloadできるvendored cordis plugin（`hmr`、console loggerなど）はupstream sourceに固定されているため（[vendoring policy](../vendor/README.md)）、このcatalogには掲載しません。
