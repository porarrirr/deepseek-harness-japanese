...
[English](persistence-catalog.md) | [中文](persistence-catalog.zh.md) | 日本語


sessionのdurable event logに現れるすべてのevent typeを掲載します。永続化される完全な`SessionEvent` envelopeと、merge-extensibleな`SessionEventMap`の各memberです。`@deepseek-ai/dsh-session`が所有する語彙に加え、このrepoで`@deepseek-ai/dsh-session/types`へdeclaration mergeするすべてのpluginを対象とし、source JSDoc、完全なpayload declaration、surface badge、declaration siteを示します。[session.md](subsystems/session.md)（surface orderと`deriveMessages()` projection）、[persistence.md](subsystems/persistence.md)（logをdurableにする方法）、[session.md](subsystems/session.md#cordis-surface)の生成領域（live bus wiring。log eventはCordis eventではなく、単一の`session/event` emitを通じてlistenerに届きます）を補完します。

このファイルはsource（`scripts/gen-persistence-catalog.ts`）からGENERATEDされ、`pnpm run verify-persistence-catalog`（`doc-sync`の一部）でfreshnessを検証します。手動で編集しないでください。declaration blockはsource declarationとnested property JSDocを保持し、containing interface／moduleによるindentationだけを除去して、`ts persistence-catalog` fenceを使います（declarationが所有moduleの型を参照するため、doc-typecheckはskipします）。payload内のtype nameは、それを説明するpageへlinkします。[persistence-log-catalog Agent Note](../.agents/notes/archived/process/2026-07-04-persistence-log-catalog.md)を参照してください。

以下のenvelope declarationは各eventの`type`、monotonic `seq`、epoch-ms `time`、`data`、任意のunknown-type skip marker `ignorable`、条件付きの`surfaceOp`／`sourceEventSeqs` fieldを組み立てます。**surface**は`SurfaceEventType` memberを示します。LLM messageを生成し、surface listへの参加方法を宣言します。**log-only**はその他すべてを示します。derived historyに寄与しないdurableでreplay可能なrecordです。すべてのpayloadはJSON-serializableです（`Session.append`で強制）。全formatは`SESSION_FORMAT_VERSION = 0`に固定されています。pre-releaseであり、compatibilityは保証しません（[version stance](subsystems/persistence.md)）。対象はこのrepoのpackageです。downstream pluginはさらにevent typeをmergeできますが、このcatalogの対象外です。
